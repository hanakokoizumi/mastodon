...
module OmniAuth
  module Strategies
    ...
    module OpenidConnectEndSessionEnhancement
      ID_TOKEN_HINT_SESSION_KEY = 'omniauth.openid_connect.id_token_hint'.freeze
      ...
      def end_session_uri
        ...
        if options.post_logout_redirect_uri.present?
          pairs['post_logout_redirect_uri'] = options.post_logout_redirect_uri
        end

        if client_options.identifier.present?
          pairs['client_id'] = client_options.identifier
        end
